...
//                 ) : (
//                     response.map((data) => (
//                         <div key={data.image_url} className={selectedImage === data.image_url ? "border-3 border-blue-500" : ""}>
//                             <button onClick={() => setSelectedImage(data.image_url)}>
//                                 <Image data={data} alt={data.image_url} /> {/* Ensure Image has alt text */}
//                             </button>
//                         </div>
//                     ))
//                 )}
        ...
        <>
            ...
            <div className="grid h-50 md:grid-cols-2 lg:grid-cols-3 xl-grid-cols-4 gap-4 my-10 max-w-7x1 mx-auto px-2 overflow-y-scroll">
                ...
                {isLoading ? (
                    ...
                    response.map((data) => (
                        <div key={data.image_url} className={selectedImage === data.image_url ? "border-3 border-blue-500" : ""}>
                            <button onClick={() => setSelectedImage(data.image_url)} aria-label={`Select image ${data.image_url}`}>
                                <Image data={data} alt={`Image of ${data.image_url}`} /> {/* Ensure Image has alt text */}
                            </button>
                        </div>
                    ))
                )}
